...
function [Aeq,qnt_coluna_MA_incidencia,qnt_coluna_MA,aux,A,barraDefeitoFalha] = MatrizA(M)
    ...
    // Definição inicial da matriz responsável pela definição dos limtes das linhas de contingência.
    matrizLimite=[];
    
    // Criação da variável auxiliar para criação da matriz que receberá a informação de quais ramos são abertos.
    a=1;
    
    // Estrutura de repetição para busca por todo arquivo de entrada da informação de ramos abertos.
    for i=2:(NR+1)
        if M(i,8)==1
            ...
            // inserção de dados na matriz referente aos limites da rede, afim de criar uma equação que associa as variáveis inteiras com as não inteiras.
            matrizLimite(a,a)=-5;
    ...
    A=cat(2,A,zeros(size(A,'r'),2*M(1,3))); // Expansão da matriz incidência para restrições de inegualdade.
...
// completo de rede, informação de existencia de  Defeito Falha.
// Saída : Matriz de carga b.
//---------------------------------------------------------------------

function [beq,b] = MatrizB(M,A,barraDefeitoFalha)
    ...
    // Verificação da existência de existir ou não defeito falha.
    if barraDefeitoFalha==0 then // Não houve Defeito Falha.
...
function [H]=MatrizH(M,qnt_coluna_MA_incidencia,aux)
...
// Saída : Vetor informação de onde ocorreu Defeito Falha.
//---------------------------------------------------------------
...
function [barraDefeitoFalha]=restricao(M)
    while 1>0 do
        ...
        // Verificação de existência de Defeito Falha.
...
[Aeq,qnt_coluna_MA_incidencia,qnt_coluna_MA,aux,A,barraDefeitoFalha]=MatrizA(M);
...
[beq,b]=MatrizB(M,A,barraDefeitoFalha);
...
// Identificação de qual Ramo foi ligado para que se chegue na convergência da rede.
ondeTaNaMatrizM=find(M(:,8)==1);
qualLigou=find(xopt(intcon(1,1):intcon(1,size(ondeTaNaMatrizM,"c")),1)==1);
lugarNaMatriz=ondeTaNaMatrizM(1,qualLigou);
disp("O ramo ativado tem como origem e destino as barras que se seguem.");
disp(M(lugarNaMatriz,1),M(lugarNaMatriz,2));
