...
//-----------------------
function [C,qnt_coluna_MA_incidencia,qnt_coluna_MA] = MatrizA(M,LE)
...
function [b] = MatrizB(M,quantosLacos)
    ...
    if(NB==NR)// Fora necessário inserir esta comparação pois no caso da rede de 400barras, haviam na verdade 402 barras e 402 ramos, mas como 1 ramo era feeder e é considerado que neste caso seja inserido uma barra nova, a quantidade de barras e ramos era diferente, ou seja 403 barras e 402 ramos no total.
        ...
        b=cat(1,b,zeros(M(1,3),1));
        // Inserção de restrições para desligamento de ramos para a parte Imaginária da rede.
        b=cat(1,b,zeros(size(b1,"r"),1));
        // Inserção de restrições de laço  pela lei de Kirchoff para a parte Imaginária da rede.
        b=cat(1,b,zeros(M(1,3)+quantosLacos,1));
    end
endfunction
...
function [Q]=MatrizQ(M,qnt_coluna_MA_incidencia)
...
[C,qnt_coluna_MA_incidencia,qnt_coluna_MA]=MatrizA(M,LE);

//Instrução para criação da matriz incidência Q.
[Q]=MatrizQ(M,qnt_coluna_MA_incidencia);
...
[b]=MatrizB(M,quantosLacos);

[x,iact,iter,f]=qpsolve(Q,p,C,b,[],[],qnt_coluna_MA)
